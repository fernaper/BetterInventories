...
public class InventoryManager {
	...
	public static void sortInventory(Container container) {
		// From ID -> Name -> ItemStack (with Item and amount)
		// Because of Anvil...
		HashMap<Integer, HashMap<String, ItemStack>> inventoryData = new HashMap<Integer, HashMap<String, ItemStack>>();
		List<ItemStack> unstackableItems = new ArrayList<ItemStack>();
		...
		List<ItemStack> inventory = container.getInventory().subList(0, container.getInventory().size() - 9*4);			
		...
		for (Entry<Integer, HashMap<String, ItemStack>>  entry : inventoryData.entrySet()) {
			System.out.print(entry.getKey() + " -> ");
			for(Entry<String, ItemStack> inner : entry.getValue().entrySet()) {
    	...
    	for (int stackId : stackIds) {
    		...
    		for (String stackName : stackNames) {
	    		ItemStack stack = inventoryData.get(stackId).get(stackName);
	    		slot = placeItemStacks(container, slot, stack);
    		}
    	}
    	...
    	// Clean rest of inventory
    	for (int i = slot; i < inventory.size(); i++) {
    		//inventory.get(i).setCount(0);
    		inventory.remove(i);
    	}
    	
    	//container.getInventory().replaceAll(inventory);
	}
	...
	private static int placeItemStacks(Container container, int slot, ItemStack stack) {
		int numItems = stack.getCount();
		int maxStackSize = stack.getMaxStackSize();
		
		int numStacks = numItems / maxStackSize;
		System.out.println(slot + " " + numItems + " " + numStacks);
		// Aggregate all stacks of the same type
		for (int i = 0; i < numStacks; i++) {
			container.getInventory().set(slot, createItemStack(stack, maxStackSize));
			slot++;
		}
		
		// Extra blocks of the same stack
		if (numItems % maxStackSize > 0) {
			System.out.println("lo pongo en "+ slot);
			container.getInventory().set(slot, createItemStack(stack, numItems % maxStackSize));
			slot++;
		}
